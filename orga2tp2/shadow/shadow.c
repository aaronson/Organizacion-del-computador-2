 ...
 */
GLfloat* samplelight(GLuint samples, GLuint size, GLfloat light[4])
{
...
shadow(GLuint list, GLfloat plane[4], GLfloat light[3])
{
  ...
  caccumsm += tscl;
  	if (ccountersm == 30){
		caccumsm /= ccountersm;
		printf ("El promedio de los ciclos empleados en c para la funcion shadowmatrix es: %u\n",caccumsm); 
	}
}
  if (onAsm == 1) {
	asm_shadowmatrix(matrix, plane, light);
  __asm__ __volatile__ ("rdtsc;sub %0,%%eax;mov %%eax,%0" : : "g" (tscl));
  asmcountersm++;
  asmaccumsm += tscl;
  	if (asmcountersm == 30){
			asmaccumsm /= asmcountersm;
			printf ("El promedio de los ciclos empleados en asm para la funcion shadowmatrix es: %u\n", asmaccumsm); 
		}
	}
...
models()
{
  ...
  couch = glmReadOBJ("data/unknown_trinity_obj.obj");
  glmUnitize(couch);
...
init(void)
{
  ...
  if (onAsm == 0) {
	...
	if (onAsm == 1) { 
		light_samples = asm_samplelight(num_samples, light_size, light_position);
	}
...
keyboard(char key, int x, int y)
{
  ...
  case 'h':
    ...
    printf("a            -  ASM ON / C OFF\n");
    printf("c            -  C ON / ASM ON\n");
    ...
    printf("e            -  Evaluate sampleLights time\n");
  ...
  case 'l':
   ...
   case 'e':
	...
	if (onAsm == 0) {
		int pepe;
		__asm__ __volatile__ ("rdtsc;mov %%eax,%0" : : "g" (pepe));
		for (ccountersl=0; ccountersl<30; ccountersl++){
			light_samples = samplelight(num_samples, light_size, light_position);
		}
		__asm__ __volatile__ ("rdtsc;sub %0,%%eax;mov %%eax,%0" : : "g" (pepe));
	
		pepe /= 30;
		printf ("El promedio de los ciclos empleados en c para la funcion samplelight con %u samples es: %u\n", num_samples, pepe); 
	...
	if (onAsm == 1) { 
		int pepe;
		__asm__ __volatile__ ("rdtsc;mov %%eax,%0" : : "g" (pepe));
		for (asmcountersl=0; asmcountersl<30; asmcountersl++){
			light_samples = asm_samplelight(num_samples, light_size, light_position);
		}
		__asm__ __volatile__ ("rdtsc;sub %0,%%eax;mov %%eax,%0" : : "g" (pepe));
		pepe /= 30;
		printf ("El promedio de los ciclos empleados en asm para la funcion samplelight con %u samples es: %u\n", num_samples, pepe);
			}
...
main(int argc, char** argv)
{
  ...
  glutCreateWindow("Orga2 TP2 : Shadows!");
